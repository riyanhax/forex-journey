...
//+------------------------------------------------------------------+
#property copyright "Copyright 2020, Mahmoud_Mart"
      ...
      if(OrderSelect(i, SELECT_BY_POS,MODE_TRADES)){
         if(OrderSymbol() == symbol && OrderType() == OP_BUY && OrderComment() == ea_comment){
            if(Ask <= OrderOpenPrice() - pips_dist*points){
               RefreshRates();
               ticket = OrderSend(symbol, OP_BUY, standard_lot_size*MathPow(multiplier,buynum_pos), Ask, 1, 0.0, Ask + takeproft*points, ea_comment, magic_num);
               buytpmod = True;
            }
            break;
         }
      ...
      if(OrderSelect(i, SELECT_BY_POS,MODE_TRADES)){
         if(OrderSymbol() == symbol && OrderType() == OP_SELL && OrderComment() == ea_comment){
            if(Bid >= OrderOpenPrice() + pips_dist*points){
               RefreshRates();
               ticket = OrderSend(symbol, OP_SELL, standard_lot_size*MathPow(multiplier,sellnum_pos), Bid, 1, 0.0, Bid - takeproft*points, ea_comment, magic_num);
               selltpmod = True;
            }
            break;
         }
...
int FindOrder(string symbol, string orderComment)
{
   int orderTicket=-1;
   ...
   for(int i=0;i<OrdersTotal();i++)
   {
     ...
     {
       if(symbol == OrderSymbol() && OrderComment() == orderComment)
       {
