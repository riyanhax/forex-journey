...
extern int znak=4;            //если 5, то пятизнаковый дилцентр
...
extern double Delta1       =80;     //первая дельта от цены для стопового ордера
extern int Delta           = 60;        //расстояние между отложенными ордерами  
extern int TP              =25;  //тейк профит в пунктах для первой сделки
extern int tral               =10;//трейлинг стоп в пунктах для первого ордера
extern int step               =1;//шаг трейлинга в пунктах
extern int Profit             =10;//профит в пунктах для пачки ордеров
extern double f            =40;  //параметр границы флета по ADX
extern double bar        = 2;   //сдвиг по барам ADX
extern double st        =25;//уровень перекупленности/перепроданности
extern double st_r      =20;//разница между линиями стохастика, когда можно ставить стоповики
//extern double hl        =150;//разница между хай и лоу один бар назад
...
extern bool Fr_Stop=true; //если true, то в пятницу не торгуем 
...
if(b==IDCANCEL) return(0);
   ...
   // Print("это 0",hl0,"это 1 ",hl1,"это исходный ",hl*Point);
    //if(hl0>=hl*Point || hl1>=hl*Point || DayOfWeek()==5)return(0);
    if(Fr_Stop==true) if(DayOfWeek()==5)return(0);
      if(iflet<f && ibuy>isell && stoh>stoh_sign)//условие для покупки и определение флета
      {
               Lots=lot;
               ticket=OrderSend(Symbol(),OP_BUY,Lots,Ask,3,0,Ask+TP*Point,"",magic,0,Blue);
                if(ticket==-1)return(0);
      ...
      else if(iflet<f && ibuy<isell && stoh<stoh_sign)//условие для продажи и определение флета
      {
                  Lots=lot;
                  ticket=OrderSend(Symbol(),OP_SELL,Lots,Bid,3,0,Bid-TP*Point,"",magic,0,Red); 
                   if(ticket==-1)return(0);
       ...
       else if(iflet>f && ibuy>isell && stoh>st && stoh<100-st && stoh>stoh_sign && stoh-stoh_sign>st_r)//условие для покупки и определение тренда
      {
                   Lots=lot;
                   ticket=OrderSend(Symbol(),OP_BUY,Lots,Ask,3,0,Ask+TP*Point,"",magic,0,Blue);
                    if(ticket==-1)return(0);
       ...
       else if(iflet>f && ibuy<isell && stoh>st && stoh<100-st && stoh<stoh_sign && stoh_sign-stoh>st_r)//условие для продажи и определение тренда
      {
                  Lots=lot;
                  ticket=OrderSend(Symbol(),OP_SELL,Lots,Bid,3,0,Bid-TP*Point,"",magic,0,Red); 
                   if(ticket==-1)return(0);
