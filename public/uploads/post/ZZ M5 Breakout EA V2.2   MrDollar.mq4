...
return(0);
 ...
 int start()
   ...
   if(orderscnt(OP_BUY,Magic)==1 && orderscnt(OP_SELLSTOP,Magic)==0){
    OrderSend(Symbol(),OP_SELLSTOP,LastOrderLots(OP_BUY,Magic)*Multiplier,NormalizeDouble(Order_Info(1,OP_SELLSTOP,"OP",Magic),Digits),3*Q,Order_Info(1,OP_SELLSTOP,"SL",Magic),Order_Info(1,OP_SELLSTOP,"TP",Magic),"MR.dollar EA",MagicNumber,0,Red);
   }
  if(orderscnt(OP_SELL,Magic)==1&&orderscnt(OP_BUYSTOP,Magic)==0){
    OrderSend(Symbol(),OP_BUYSTOP,LastOrderLots(OP_SELL,Magic)*Multiplier,NormalizeDouble(Order_Info(0,OP_BUYSTOP,"OP",Magic),Digits),3*Q,Order_Info(0,OP_BUYSTOP,"SL",Magic),Order_Info(0,OP_BUYSTOP,"TP",Magic),"MR.dollar EA",MagicNumber,0,Blue);
    }
  }   
 }
    ...
    if (Signal&&Time0!=Time[0]){
     if(Num<MaxMainOrders&&orderscnt(OP_BUYSTOP,0)+orderscnt(OP_SELLSTOP,0)==0){
        ...
        double sellprice=MathMin(GetZigZag(3),GetZigZag(4));
         ...
         OrderSend(Symbol(),OP_BUYSTOP,Lots,NormalizeDouble(buyprice+3*point,Digits),3*Q,SL,TP,"MR.dollar EA",MagicNumber,0,Blue);
         SL=buyprice+3*point;
         ...
         OrderSend(Symbol(),OP_SELLSTOP,Lots,NormalizeDouble(sellprice-3*point,Digits),3*Q,SL,TP,"MR.dollar EA",MagicNumber,0,Red);
         PlaySound("Alert.wav");
