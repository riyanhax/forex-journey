...
int start()
{
   ...
   limit = MathMax(limit,TimeFrame/Period()+1);   
      ...
      UpTrendSAR[shift] = iCustom(NULL,TimeFrame,IndicatorName,0,Price,HiLoMode,StartAF,MinStep,MaxStep,MaxAF,AdaptiveMode,AdaptiveSmooth,Filter,MinChange,SignalMode,
                                  "",AlertMode,SoundsNumber,SoundsPause,UpSound,DnSound,EmailMode,EmailsNumber,0,y);     
      DnTrendSAR[shift] = iCustom(NULL,TimeFrame,IndicatorName,0,Price,HiLoMode,StartAF,MinStep,MaxStep,MaxAF,AdaptiveMode,AdaptiveSmooth,Filter,MinChange,SignalMode,
                                  "",AlertMode,SoundsNumber,SoundsPause,UpSound,DnSound,EmailMode,EmailsNumber,1,y);     
              
         if(SignalMode > 0)
         { 
         UpSignal[shift] = iCustom(NULL,TimeFrame,IndicatorName,0,Price,HiLoMode,StartAF,MinStep,MaxStep,MaxAF,AdaptiveMode,AdaptiveSmooth,Filter,MinChange,SignalMode,
                                   "",AlertMode,SoundsNumber,SoundsPause,UpSound,DnSound,EmailMode,EmailsNumber,2,y);      
         DnSignal[shift] = iCustom(NULL,TimeFrame,IndicatorName,0,Price,HiLoMode,StartAF,MinStep,MaxStep,MaxAF,AdaptiveMode,AdaptiveSmooth,Filter,MinChange,SignalMode,
   ...
   for(int shift=limit;shift>=0;shift--) 
   {
      ...
      AF[shift]    = AF[shift+1];
         ...
         if(HiLoMode > 0) double inprice = iMA(NULL,0,1,0,0,4,shift); else inprice = hiprice[0];
            ...
            if(AdaptiveMode == 2)
            {
               if(UpTrendSAR[shift+1] != EMPTY_VALUE) inputs[shift] = MathAbs(inprice - UpTrendSAR[shift+1]); 
               else 
               if(DnTrendSAR[shift+1]!= EMPTY_VALUE) inputs[shift] = MathAbs(inprice - DnTrendSAR[shift+1]);
            }
